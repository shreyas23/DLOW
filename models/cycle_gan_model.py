...
class CycleGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B', 'D_A_classification', 'D_B_classification', 'D_A_classification_0', 'D_B_classification_0', 'G_A_classification', 'G_B_classification', 'G_A_classification_0', 'G_B_classification_0']
        ...
        if self.isTrain:
            self.model_names = ['G_A', 'G_B', 'D_A', 'D_B', 'D_A_classification', 'D_B_classification', 'D_A_classification_0', 'D_B_classification_0', 'DeConv']
        ...
        self.netG_A = networks.define_stochastic_G(nlatent=opt.nlatent, input_nc=opt.input_nc,
                                                     output_nc=opt.output_nc, ngf=64,
        ...
        if self.isTrain:
            ...
            self.netD_A = networks.define_D_B(input_nc=opt.output_nc,
            ...
            self.netD_B = networks.define_D_B(input_nc=opt.output_nc,
                                              ndf=opt.ndf, which_model_netD=opt.which_model_netD,
                                              norm=opt.norm, use_sigmoid=opt.use_sigmoid, gpu_ids=opt.gpu_ids)
            self.netD_A_classification = networks.define_D_B(input_nc=opt.output_nc,
                                              ndf=opt.ndf, which_model_netD=opt.which_model_netD,
                                              norm=opt.norm, use_sigmoid=opt.use_sigmoid, gpu_ids=opt.gpu_ids)
            self.netD_A_classification_0 = networks.define_D_B(input_nc=opt.input_nc,
                                              ndf=opt.ndf, which_model_netD=opt.which_model_netD,
                                              norm=opt.norm, use_sigmoid=opt.use_sigmoid, gpu_ids=opt.gpu_ids)
            self.netD_B_classification = networks.define_D_B(input_nc=opt.input_nc,
                                              ndf=opt.ndf, which_model_netD=opt.which_model_netD,
                                              norm=opt.norm, use_sigmoid=opt.use_sigmoid, gpu_ids=opt.gpu_ids)
            self.netD_B_classification_0 = networks.define_D_B(input_nc=opt.input_nc,
                                              ndf=opt.ndf, which_model_netD=opt.which_model_netD,
                                              norm=opt.norm, use_sigmoid=opt.use_sigmoid, gpu_ids=opt.gpu_ids)
        ...
        if self.isTrain:
            ...
            self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_B.parameters()),
                                                lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizer_D_classification = torch.optim.Adam(itertools.chain(self.netD_A_classification.parameters(), self.netD_B_classification.parameters()),
                                                lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizer_D_classification_0 = torch.optim.Adam(itertools.chain(self.netD_A_classification_0.parameters(), self.netD_B_classification_0.parameters()),
                                                lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizers = []
            self.schedulers = []
    ...
    def set_input(self, input, beta, alpha, sign):
        ...
        if self.isTrain:
            ...
            if sign == '0':
                ...
                print("Setinput: Error occur when getting the 0, 1, 0to1")
            self.add_item = self.netDeConv(Variable(torch.FloatTensor([self.random_number]).view(1,1,1,1)).cuda(self.gpu_ids[0], async=True))
        else:
            # self.add_item = np.ones((1, self.nlatent, 1, 1))*self.label_intensity
            # self.add_item = torch.FloatTensor(self.add_item)
            self.add_item = self.netDeConv(Variable(torch.FloatTensor([self.label_intensity]).view(1,1,1,1)).cuda(self.gpu_ids[0], async=True))
    ...
    def backward_D_A(self):
        ...
        self.loss_D_A = self.backward_D_Regbasic(self.netD_A, self.real_B, self.real_A, 1)

    def backward_D_B(self):
        # fake_A = self.fake_A_pool.query(self.fake_A)
        # self.loss_D_B = self.backward_D_basic(self.netD_B, self.real_A, self.real_B, 1)
        self.loss_D_B = self.backward_D_Regbasic(self.netD_B, self.real_A, self.real_B, 1)

    def backward_D_A_classification(self, random_number):
        fake_B = self.fake_B_pool.query(self.fake_B)
        self.loss_D_A_classification = self.backward_D_basic(self.netD_A_classification, self.real_B, fake_B, random_number)
        # print("backward_D_A_classification:", random_number)

    def backward_D_B_classification(self, random_number):
        fake_A = self.fake_A_pool.query(self.fake_A)
        self.loss_D_B_classification = self.backward_D_basic(self.netD_B_classification, self.real_A, fake_A, random_number)
        # print("backward_D_B_classification:", random_number)

    def backward_D_A_classification_0(self, random_number):
        fake_B = self.fake_B_pool.query(self.fake_B)
        self.loss_D_A_classification_0 = self.backward_D_basic(self.netD_A_classification_0, self.real_A, fake_B, random_number)
        # print("backward_D_A_classification_0:", random_number)

    def backward_D_B_classification_0(self, random_number):
        fake_A = self.fake_A_pool.query(self.fake_A)
        self.loss_D_B_classification_0 = self.backward_D_basic(self.netD_B_classification_0, self.real_B, fake_A, random_number)
    ...
    def backward_G(self):
        ...
        self.loss_G_A_classification_0 = self.criterionGAN(self.netD_A_classification_0.forward(self.fake_B), True)*lambda_GA_classification_0*(1-self.random_number)
        self.loss_G_A = self.criterionRegression(self.netD_A.forward(self.fake_B), self.random_number)*lambda_GA
        self.loss_G_A_classification = self.criterionGAN(self.netD_A_classification.forward(self.fake_B), True)*lambda_GA_classification*self.random_number



        # GAN loss D_B(G_B(B))
        self.fake_A = self.netG_B.forward(self.real_B, self.add_item)
        self.loss_G_B_classification_0 = self.criterionGAN(self.netD_B_classification_0.forward(self.fake_A), True)*lambda_GB_classification_0*(1-self.random_number)
        self.loss_G_B = self.criterionRegression(self.netD_B.forward(self.fake_A), self.random_number)*lambda_GB
        self.loss_G_B_classification = self.criterionGAN(self.netD_B_classification.forward(self.fake_A), True)*lambda_GB_classification*self.random_number
        ...
        # combined loss
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.loss_G_A_classification + self.loss_G_B_classification + self.loss_G_A_classification_0 + self.loss_G_B_classification_0
    ...
    def optimize_parameters(self):
        ...
        self.backward_D_A()
        self.backward_D_B()
        gnorm_D = torch.nn.utils.clip_grad_norm(itertools.chain(self.netD_A.parameters(), self.netD_B.parameters()), self.opt.max_gnorm)
        self.optimizer_D.step()
        # G_A and G_B
        self.optimizer_G.zero_grad()
        self.backward_G()
        gnorm_G = torch.nn.utils.clip_grad_norm(itertools.chain(self.netG_A.parameters(), self.netG_B.parameters()), self.opt.max_gnorm)
        ...
        gnorm_D_classification = torch.nn.utils.clip_grad_norm(itertools.chain(self.netD_A_classification.parameters(), self.netD_B_classification.parameters()), self.opt.max_gnorm)
        ...
        gnorm_D_classification_0 = torch.nn.utils.clip_grad_norm(itertools.chain(self.netD_A_classification_0.parameters(), self.netD_B_classification_0.parameters()), self.opt.max_gnorm)
